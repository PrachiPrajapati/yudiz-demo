<?php
/**
** A base module for [phonenumbercust] and [phonenumbercust*]
**/

/* form_tag handler */

add_action( 'wpcf7_init', 'yspl_add_form_tag_phonenumbercust' );

function yspl_add_form_tag_phonenumbercust() {
	wpcf7_add_form_tag(
		array( 'phonenumbercust', 'phonenumbercust*'),
		'yspl_phonenumbercust_form_tag_handler', array( 'name-attr' => true ) );
}

function yspl_phonenumbercust_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'wpcf7_validate_phonenumbercust', 'yspl_phonenumbercust_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_phonenumbercust*', 'yspl_phonenumbercust_validation_filter', 10, 2 );

function yspl_phonenumbercust_validation_filter( $result, $tag ) {
	$type = $tag->type;
	$name = $tag->name;
...
add_action( 'wpcf7_admin_init', 'yspl_add_tag_generator_phonenumbercust', 20 );

function yspl_add_tag_generator_phonenumbercust() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'phonenumbercust', __( 'phone number Yudiz', 'nb-cpf' ),
		'yspl_tag_generator_phonenumbercust' );
}

function yspl_tag_generator_phonenumbercust( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
	$type = 'phonenumbercust';
